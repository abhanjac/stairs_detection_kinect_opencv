# Objective: 
This project is about the detection of stairs using a 3D camera in real-time for a prosthetic leg application. Although there are several smart actuated prosthetic legs in the market, they mostly do not have any automated gait modification mechanism built into them. That means the person using this leg has to do some kind of manual movement to switch the leg from walking gait to stairs climbing move when he is about to go up or down a stair. This project uses a Microsoft Kinect depth camera and Opencv libraries to detect stairs in real-time using the depth and RGB images from the camera. Since, the overall algorithm should be capable of running on some single-board micro-computer installed in the prosthetic leg, so no neural networks were used in the image processing. This project only involves proving and testing out the vision-based software for the leg and does not involve any interfacing with the hardware. The overall failure rate was 1% for the algorithm.

**A *Trailer* of Final Result:**

| | |
|:---:|:---:|
| <img src="https://github.com/abhanjac/stairs_detection_kinect_opencv/blob/master/testing_videos/vid_real_up_stairs.gif" width="160" height="120"> | <img src="https://github.com/abhanjac/stairs_detection_kinect_opencv/blob/master/testing_videos/vid_real_dn_stairs.gif" width="160" height="120"> |
...
So, when a patient or a user of these prosthetic legs is walking towards a set of stairs, he or she must manually switch over  
...
# Current Framework: 
**[ Note: Please also check Future Improvements section. ]** 
